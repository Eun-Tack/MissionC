...
| Organization/business/project hierarchy | FR-ORG-01, FP-3 | `organizations`, `businesses`, `projects`, `project_stages` | `src/core_api/routers/hierarchy.py`, `hierarchy.html`, `project.html` | hierarchy page smoke, FK tests | Built, needs data policy decisions |
...
| Radial command palette | SDG-DEC-003, F-2, F-4 | command state in browser | `radial.js`, `radial.css`, `radial_palette.html` | GitHub issue #1 | Disabled/deferred |
...
|---------|------|-------------|----------------|
| HOLE-001 | Product naming | Code still uses `MC` in title, env vars, DB names, comments. | Should MissionC remain product-only while `MC_*` stays internal, or should we migrate code names too? |
| HOLE-002 | Organization model | Calendar/items now expose organization/project linkage, but the exact rule for "affiliation" is not final. | Is affiliation always organization -> business -> project, or can an item belong directly to an organization without a project? |
| HOLE-003 | Palette | Palette is implemented but user-reported behavior makes it unsafe to keep global. | Should palette be disabled until fixed, or kept behind a visible experimental toggle? |
| HOLE-004 | Sync scheduler | Settings include poll intervals, while local architecture uses on-demand sync except inbox expiry. | Should the UI say on-demand/local, or do you want background sync reintroduced? |
...
| HOLE-006 | Tests | Current tests cover core flows but not several newer routers. | Which workflows are most critical to protect first: calendar, hierarchy, contacts, or integrations? |
